...
class image_obj_tracked {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new image_obj_tracked(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.rect_ranged[i] = image_rect_ranged.deserialize(buffer, bufferOffset)
    }
  ...
  static Resolve(msg) {
    ...
    const resolved = new image_obj_tracked(null);
    ...
    if (msg.rect_ranged !== undefined) {
      ...
      for (let i = 0; i < resolved.rect_ranged.length; ++i) {
        resolved.rect_ranged[i] = image_rect_ranged.Resolve(msg.rect_ranged[i]);
      }
    }
    else {
...
module.exports = image_obj_tracked;
