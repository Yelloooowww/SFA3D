...
class vscan_tracked_array {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.obj_tracked.forEach((val) => {
      bufferOffset = vscan_tracked.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new vscan_tracked_array(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.obj_tracked[i] = vscan_tracked.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new vscan_tracked_array(null);
    ...
    if (msg.obj_tracked !== undefined) {
      ...
      for (let i = 0; i < resolved.obj_tracked.length; ++i) {
        resolved.obj_tracked[i] = vscan_tracked.Resolve(msg.obj_tracked[i]);
      }
    }
    else {
...
module.exports = vscan_tracked_array;
